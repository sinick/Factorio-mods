...
local chest_count = 12
local chest_prefix = "NCA-Chest-"
...
local function on_selected_area(event)
	...
	if event.item ~= "sc-merge-tool"  then
		return
	end	
	...
	if local_purge_index > chest_count then
		local_purge_index = 1
	end
end	
...
local entity_filter = {}
for i=1, chest_count do
	table.insert(entity_filter, {filter='name', name=chest_prefix .. i})
end
